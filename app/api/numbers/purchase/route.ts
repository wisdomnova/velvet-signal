...
export async function POST(request: NextRequest) {
  try {
    ...
    // ✅ HYBRID CONFIGURATION: Purchase number with DIRECT webhook configuration
    // This enables incoming calls to ring in the browser automatically
    // TwiML App is still used separately for outbound browser calls
    const number = await twilioClient.incomingPhoneNumbers.create({
      phoneNumber,
      voiceUrl: `${baseUrl}/api/voice/webhook`,         // 🔗 DIRECT webhook for incoming calls
      voiceMethod: 'POST',
      smsUrl: `${baseUrl}/api/sms/webhook`,            // 🔗 DIRECT webhook for incoming SMS  
      smsMethod: 'POST',
      statusCallback: `${baseUrl}/api/calls/status`,   // 🔗 Status updates for call completion
      statusCallbackMethod: 'POST'
    });

    console.log('✅ Number purchased with DIRECT webhooks configured:', {
      phoneNumber: number.phoneNumber,
      sid: number.sid,
      voiceUrl: `${baseUrl}/api/voice/webhook`,         // This handles INCOMING calls directly
      smsUrl: `${baseUrl}/api/sms/webhook`,            // This handles INCOMING SMS directly
      statusCallback: `${baseUrl}/api/calls/status`,   // This handles call completion data
      capabilities: number.capabilities
    });

    // Save to database with comprehensive webhook tracking
    const { data: savedNumber, error } = await supabase
      .from('phone_numbers')
      .insert({
        sid: number.sid,
        ...
        status: number.status,
        voice_url: `${baseUrl}/api/voice/webhook`,      // Track webhook URLs in database
        sms_url: `${baseUrl}/api/sms/webhook`,
        status_callback: `${baseUrl}/api/calls/status`,
        date_created: new Date().toISOString(),
        date_updated: new Date().toISOString(),
    ...
    console.log('💾 Number saved to database with automatic webhook configuration:', {
      sid: savedNumber.sid,
      phoneNumber: savedNumber.phone_number,
      userId: savedNumber.user_id,
      voiceUrl: savedNumber.voice_url,
    ...
    return NextResponse.json({
      success: true,
      message: 'Number purchased successfully with automatic webhook configuration for incoming calls',
      number: {
        id: savedNumber.id,
        sid: savedNumber.sid,
        phoneNumber: savedNumber.phone_number,
